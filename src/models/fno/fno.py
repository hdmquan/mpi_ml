...
if __name__ == "__main__":
    ...
    coords = model.get_grid((height, width), device)
    ...
    model_no_coords = FNOModel(
        in_channels=in_channels, out_channels=out_channels, include_coordinates=False
    ).to(device)

    output_no_coords = model_no_coords(x)
    print(f"Output shape (no coords): {output_no_coords.shape}")

    assert output_no_coords.shape == expected_shape
